...
class HttpGetReader extends Readable {
  ...
  _onRequestError (error) {
    logger.log('error', 'Error while making HTTP GET request to \'%s\': %s', this._url, error)
  ...
  _onTransformError (error) {
    logger.log('error', 'Error while transforming data from HTTP GET request to \'%s\': %s', this._url, error)
